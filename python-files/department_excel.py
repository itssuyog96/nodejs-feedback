...
for i in jsondata:
    for j in i:
        if(j == 'lab_id'):
            lab_count += 1
            col_y += sub + height
            worksheet.write(col_y-1, 2, 'LAB NAME', header_format)
            worksheet.write(col_y-1, 3, 'SUBJECT NAME', header_format)
            sub = 0
            if_report = False
            #append new table
...
formula = "".join(f)
print(formula)
worksheet.write((height * lab_count) + sub_count + height, 3, 'DEPARTMENT LAB AVERAGE FINAL RATING', header_format)
worksheet.write_formula((height * lab_count) + sub_count + height, 4, formula, bold)
